...
var prefix = "$"
    ...
    if(rokz.content.startsWith(prefix + "تقديم")) {
     ...
     .then(co => {
       ...
       e.edit(`ما هوا الفرق بين var و conset
[${lang}]`)
...
client.on('guildMemberAdd', member => {
 ...
 client.on("voiceStateUpdate", (old, Updates) => {
    var channel = "517169904605069333";
    var role = "DNS-Organ"
    لول(old,Updates,channel,role);
});

function لول(o,n,channel,role){
    if (!o.voiceChannel && n.voiceChannel) {
        if (n.voiceChannelID == channel) {
            n.addRole(n.guild.roles.find("name", '• Mal » Members'));
        };
...
client.on('html',async html => {
    if(html.content.startsWith(code + "er")) {
  if(!html.channel.guild) return html.reply('This Command For Servers Only !');
    let jscodes = html.guild.channels.find(`name`, "✵-「discord-erice");
    if(!jscodes) return html.channel.send(":x:لم اجد الروم الخاص بنشر الاكواد");
      let filter = m => m.author.id === html.author.id;
      let thisMessage;
      let thisFalse;
...
client.on('html',async html => {
    if(html.content.startsWith(code + "file")) {
  if(!html.channel.guild) return html.reply('This Command For Servers Only !');
    let jscodes = html.guild.channels.find(`name`, "✵-「files_js");
    if(!jscodes) return html.channel.send(":x:لم اجد الروم الخاص بنشر الاكواد");
      let filter = m => m.author.id === html.author.id;
      let thisMessage;
      let thisFalse;
      html.channel.send(':pencil: **| من فضلك اكتب رابط الملف الان... :pencil2: **').then(msg => {
